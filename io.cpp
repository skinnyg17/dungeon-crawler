...
void look_monsters(dungeon_t *d) {
  //TODO
  pair_t dest;
  int c;
  fd_set readfs;
  ...
  mvprintw(0, 0,
           "Choose a location use 't'. quit using esc.");
  ...
  * not otherwise used.                                                 */
  ...
  if(d->character_map[dest[dim_y]][dest[dim_x]] != d->PC && d->character_map[dest[dim_y]][dest[dim_x]]) {
    WINDOW *popupm = newwin(18, 78, 1, 1);
    box(popupm, 0, 0);
    wrefresh(popupm);
    mvwprintw(popupm, 1, 1, "%s", "Description:");
    mvwprintw(popupm, 3, 3, "%s", get_desc(dynamic_cast<npc *>(d->character_map[dest[dim_y]][dest[dim_x]])));
    wrefresh(popupm);
    c = getch();
    if(c == 27) {
      delwin(popupm);
    ...
    io_queue_message("Not a monster!!");
}
